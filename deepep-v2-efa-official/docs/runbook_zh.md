...
b300 上把 tag 换成 `:sm103`，其余一字不改。**只有这两个变量和真实运行不同**：`WORLD_SIZE=1`
和 `127.0.0.1`；node rank 仍然是 `0`，而 `WORLD_SIZE` 的默认值是 **2**，所以单机必须显式传。
单机也是唯一不会踩 `ibstat` 除零的形态（§12），正因如此它测的是构建而不是网络 —— 反过来，
它**测不到**跨节点 QP 建立、线上的 GIN type 5、以及 combine 的节点分层（§9.4），所以单机
的数字一个都不能引用。
