#!/usr/bin/env bash
# Launch DeepEP V1 tests/test_internode.py inside the deepep-v1-efa:dev image.
# Args:
...
docker run --rm \
  ...
  deepep-v1-efa:dev \
  bash -lc "cd /opt/deepep && python tests/test_internode.py --num-processes ${NUM_PROCESSES} $*"
